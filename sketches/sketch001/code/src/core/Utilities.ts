...
export interface LineGridParameters {
    ...
    // Random noise GUI controller
    gui_scaleNoise: DAT.GUIController;
    // Max noise that can be added when drawing a line
    scaleNoise_max: number;
    // Min noise that can be added when drawing a line
    scaleNoise_min: number;
    // Noise adjustment step size
    scaleNoise_step: number;
    // The currently set noise level
    scaleNoise: number;

    // nLines GUI controller
    gui_nLines: DAT.GUIController;
    // Max possible number of lines that can be plotted
    // on the canvas
    nLines_max: number;
    // Min possible number of lines that can be plotted
    // on the canvas
    nLines_min: number;
    // Line number adjustment step size
    nLines_step: number;
    // The currently set number of lines to draw
    nLines: number;
...
// Define a function to initialize and set properties
// of a "LineGrid" object
export function initialzeLineGridParameters(): LineGridParameters {
    let gridLineParameters = {
        "gui_randomSeed": undefined,
        "randomSeed": 1234,

        "gui_orientation": undefined,
        "orientation": "both",
        ...
        "gui_scaleNoise": undefined,
        "scaleNoise_max": 10.00,
        "scaleNoise_min":  0.000,
        "scaleNoise_step": 0.001,
        "scaleNoise": 2.500,

        "gui_nLines": undefined,
        "nLines_max": 500,
        "nLines_min":  2,
        "nLines_step":  1,
        "nLines": 25,
    ...
    return gridLineParameters as LineGridParameters;
}
